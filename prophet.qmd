...
```{r}
# Benchmark model - Prophet
library(prophet)

## Ajustar datos para Prophet
train_prophet <- series_tsbl |> 
  select(date, sales) |> 
...
# Realizar pronóstico
future <- make_future_dataframe(prophet_model, periods = 16)  # Hacer un dataframe de futuro para la predicción
...
rmse_value <- sqrt(mean((predictions - real_values)^2))
mae_value <- mean(abs(predictions - real_values))
mape_value <- mean(abs((predictions - real_values) / real_values)) * 100
rsq_value <- 1 - sum((predictions - real_values)^2) / sum((real_values - mean(real_values))^2)
rmsle <- sqrt(mean((log(predictions + 1) - log(real_values + 1))^2))
...
```{r}
submission |> 
  write_csv("submission_prophet.csv")
```
