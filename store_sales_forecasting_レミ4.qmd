...
series_tsbl_fit <- series_tsbl %>% 
  model(
    sarima = ARIMA(
      sales ~ pdq(0,1,1) +           # parte no estacional (p, d, q)
              PDQ(0,1,1, 7),         # parte estacional (P, D, Q)[periodo]
      stepwise      = FALSE,
      approximation = FALSE          # evita aproximaciones para un ajuste exacto
    )
  )
tictoc::toc()
...
```{r}
submission |> 
  write_csv("submission_remi4.csv")
```
